# Timing constraints for sky130_vex2_soc.
# Based on OpenLane base.sdc, with async reset treated as a timing exception.
#
# Wired via PNR_SDC_FILE / SIGNOFF_SDC_FILE in config.yaml (replaces the fallback SDC).
...
# Exclude async reset from synchronous IO delay / drive constraints.
set rst_input [get_ports reset]
set rst_indx [lsearch $all_inputs_wo_clk $rst_input]
if { $rst_indx >= 0 } {
    set all_inputs_wo_clk_rst [lreplace $all_inputs_wo_clk $rst_indx $rst_indx ""]
} else {
    puts "\[WARNING] Port 'reset' not found among inputs; continuing without reset exclusion."
