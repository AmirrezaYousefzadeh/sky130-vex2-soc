...
python3 "$ROOT/hardening/scripts/explore_area_fmax.py" --write "$@"
...
LOGDIR="$ROOT/hardening/sky130_vex2_soc/explore_logs"
mkdir -p "$LOGDIR"
...
extract_metrics() {
  local tag="$1"
  local run="$ROOT/hardening/sky130_vex2_soc/runs/$tag"
...
for name in "${NAMES[@]}"; do
  cfg="$ROOT/hardening/sky130_vex2_soc/explore_${name}.json"
  tag="explore_${name}"
  log="$LOGDIR/${name}.log"
  ...
  CFG="$cfg" RUN_TAG="$tag" TO=OpenROAD.STAPostPNR OVERWRITE=1 \
    "$ROOT/hardening/run_harden.sh" >"$log" 2>&1
  rc=$?
  set -e
  if [[ $rc -eq 0 ]]; then
    status="OK"
